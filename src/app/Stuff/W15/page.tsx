import Image from "next/image";

export default function W15() {
  return (
     <main className="flex w-full">
      <div className="flex w-full max-w-[2100px] flex-col space-y-6">

         <p className="text-xl leading-relaxed">Week fifftean 15 is available below</p>
         <p className="text-xl leading-relaxed">This week started off with a journey back to my hometown, Delaware. Driving home in the morning with the young kat sloan is always a treat. On the road, we exchange meows, scope painting locations, drink 1 to 2 coffees, and even listen to a few podcasts. I am always looking for landscape vistas that inspire me, even something as simple as how light can hit a hay field causing an array of yellows and greens.</p>
         <p className="text-xl leading-relaxed">I got to dial in a good run on the Delaware D, the weather was gorgeous and the pace was sustained. </p>
         <p className="text-xl leading-relaxed">Further down, I was able to lock down a good business meeting with a client that is super interested in the software I am learning. It was the success case that proved all my research worth it. I just have to keep focused on the internal workings of our company to push these ideas to the fullest potential.</p>
         <p className="text-xl leading-relaxed">On Friday night, the flight was booked and headed out of London to Victoria with a stop in Calgary for a quick 1 hour layover. The flight was delayed in london for 2hrs and westjet bought me a continental breakfast and an overnight stay at the airports nicest hotel. Honestly, the overnight was appreciated as I got my belly filled and arrived early the next morning with a good nights sleep. I roomed with this random guy, Rivanshu, and he made me tea and gave me some of his snacks! Always be nice to your neighbour!</p>
         <p className="text-xl leading-relaxed">Landing in Victoria after 13 hours of travel, I was instantly greeted by all the friends who we were staying with! All stoked up, we jumped into a van and headed onto the ferry to Galiano island. There the sun melted us and the west coast air filled nostrils, a tad salty!</p>
         <p className="text-xl leading-relaxed">We are here until the 7th of September for a friends wedding in Comox. A Bilbo Baggins event!</p>
         <p className="text-xl leading-relaxed">8.27.2025 nomadicness is nice</p>
      </div>
     </main>
  );
}
